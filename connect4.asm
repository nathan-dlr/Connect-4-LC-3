...
CHECK_HORI
         AND R4, R4, #0  ;R4 will count if we have a connect four; it will return 4 if it detects a connect four and a 0 if it doesn't
         ADD R4, R4, #1  ;it already has one from the piece we just placed 
         ST  R1, SAVER1
         ST  R2, SAVER2
         ST  R6, SAVER6
...
;check down diagonal subroutine          
CHECK_DD
           AND R4, R4, #0  ;R4 will count if we have a connect four; it will return 4 if it detects a connect four and a 0 if it doesn't
           ADD R4, R4, #1  ;it already has one from the piece we just placed 
           ST  R1, SAVER1
...
CHECKLFTDD LDR R1, R1, #1
           ...
           BRz WIN_DD
           BRnzp CHECKLFTDD
CHECKRHTDD LD  R1, SAVER1
           LD  R3, SAVER3
CHCKRHT2DD LDR R1, R1, #0 
           BRz NOWIN_DD
           ADD R3, R3, #1
           ADD R2, R1, R3
           LDR R6, R2, #0
...
CHECKLFTUD LDR R1, R1, #1
           ...
           BRz WIN_UD
           BRnzp CHECKLFTUD
CHECKRHTUD LD  R1, SAVER1
           LD  R3, SAVER3
CHCKRHT2UD LDR R1, R1, #0 
           BRz NOWIN_UD
           ADD R3, R3, #-1
           ADD R2, R1, R3
           LDR R6, R2, #0
